...
export default function Services() {
  return (
    <section className="relative z-30 w-full py-32 px-6 bg-gradient-to-b from-[#042b33] to-[#063b45]">
        ...
        >
          {servicesData.map((service, index) => (
            ...
            >
              ...
              <div className="w-14 h-14 rounded-full bg-white border border-white/50 flex items-center justify-center mb-6 shadow-sm group-hover:shadow-[0_0_15px_rgba(67,232,216,0.5)] transition-all duration-500">
                <service.Icon className="w-6 h-6 text-[#06828f]" strokeWidth={1.5} />
              </div>

              <h3 className="text-2xl font-display font-bold text-white mb-4">{service.title}</h3>
              <p className="text-white/95 font-sans leading-relaxed">
                {service.description}
              </p>
            </motion.div>
          ))}
